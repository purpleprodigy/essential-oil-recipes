...
function single_post_featured_image() {
	if ( ! is_singular( 'post') )
		return;
	$img = genesis_get_image( array( 'format' => 'html', 'size' => 'thumbnail', 'attr' => array( 'class' => 'alignright' ) ) );
...
function single_page_featured_image() {
	if ( ! is_singular( 'page' ) )
		return;
	$img = genesis_get_image( array( 'format' => 'html', 'size' => 'medium', 'attr' => array( 'class' => 'alignright' ) ) );
	printf( '<a href="%s" title="%s">%s</a>', get_permalink(), the_title_attribute( 'echo=0' ), $img );
...
function add_google_adsense() {
	if ( is_category( 'downloads' ) || is_category( '1' ) || is_page( '1249' ) ) {
		return;
